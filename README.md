...
- 巡回同期（パラメーター圧縮） — 指定したパラメーター群を「インデックス + 型別値チャンネル」
  という少数の同期パラメーターに時分割多重。VRChat の約 0.3 秒の同期周期を利用し、
  ローカル側の巡回ステートが Parameter Driver で値を順番に載せ、リモート側が
  AnyState デコーダーで書き戻す。同期コストのプレビュー付き
  （コントローラー概要またはパラメーターパネルの「Add」から）
- ネットワーク同期（ベータ） — ローカル専用パラメーターで動くレイヤーを、同期パラメーター
  （Int または Bool×n bit）+ Parameter Driver + リモートミラーステートの定石構成で同期化。
  生成構造は今後変わる可能性があります（レイヤーの歯車メニューまたはコントローラー概要から）
