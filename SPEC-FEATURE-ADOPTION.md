...
## 1. Network Sync ジェネレータ 【UI 撤去(2026-08-01)】

> **ステータス: UI 撤去。** レイヤー全体をミラーする本機能は生成物が大きく、実運用での
> 有用性が薄いと判断してウィザードと入口ボタンを削除した。`NetworkSyncBuilder` モデルと
> テストはコードとして残置(§12 非同期同期が IsLocal / bit エンコードのヘルパーを共有)。
> パラメーターの**値**を同期する用途は §12 の非同期同期(Async Sync)を使う。
...
## 12. 非同期同期(Async Sync / パラメーター圧縮) 【P1・追加】

### 12.1 目的

VRC の同期 Expression パラメーターはおおよそ 0.3 秒に一度リモートへ同期される。
これを利用し、N 個のパラメーターを個別に同期する(N×8bit 等)代わりに、
**インデックス + 型ごとの値チャンネル**という少数の同期パラメーターだけを同期し、
巡回ステートで時分割多重する。対象パラメーター自体は非同期のまま。

### 12.2 生成構造(1 レイヤー)

- 生成パラメーター: `IsLocal`(Bool)、インデックス(`<base>/Index` Int、または
  `<base>/Index/b0..bn` Bool×ceil(log2 N)・LSB-first)、値チャンネル
  (選択パラメーターに含まれる型ごとに `<base>/Float` / `<base>/Int` / `<base>/Bool`)。
- **ローカル送信側**: スロット i ごとに `Send <param>` ステート。
  Parameter Driver(localOnly=true、インスタンス名 `RR Send`)で
  ① Copy: 対象 i → その型のチャンネル ② Set: インデックス = i(値→インデックスの順)。
  遷移は Exit Time のみ(hasExitTime、exitTime = ステップ秒。**ステートは意図的に
  モーションなし** — モーションなしステートの正規化時間は 1/秒で進むため exitTime が
  そのまま秒として機能する。WD は ON)。i → (i+1) mod N で巡回。
- **リモート受信側**: `Remote Idle` + スロットごとの `Recv <param>` ステート。
  AnyState → Recv i(条件: IsLocal == false ∧ インデックス == i、
  canTransitionToSelf = false、即時遷移)。Recv i の Driver(localOnly=false、
  `RR Recv`)が Copy: チャンネル → 対象 i。
- **Entry**: デフォルトステート = Send 0(ローカルのフォールバック)、
  Entry 遷移 IsLocal == false → Remote Idle。
- **ストア連携**: 関連付け済みパラメーターストア(§10)がある場合、生成した同期
  パラメーターを synced / saved=false で追加するオプション(既定 ON)。対象
  パラメーター自体は追加しない。対象がストアで synced のままなら警告。

### 12.3 UI(ウィザード)

対象パラメーターのチェックリスト(Float / Int / Bool)、ベース名、インデックス
エンコード(Int / Bool×n)、ステップ間隔(秒、既定 0.3)、ストア追加トグル、
**同期コストプレビュー**(圧縮後 bit vs 個別同期 bit)。
入口: コントローラー概要 / パラメーターパネル Add メニュー。

### 12.4 既知の制約(UI の警告で明示)

- 値の反映は最大 N × ステップ秒の遅延。途中参加のリモートは 1 巡で追いつく。
- インデックスと値は別パラメーターとして同期されるため、1 ステップ分だけ古い値と
  新しいインデックスが組み合わさる瞬間があり得る。
- 同期 Float チャンネルは -1〜1・8bit 精度。範囲外の Float は伝わらない。
- ステップ間隔 < 0.3 秒はリモートの取りこぼしリスク。
- 圧縮が効くのは「同型のパラメーターが多い」場合(チャンネルを共有できるため)。
