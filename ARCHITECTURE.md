...
| **Analyze** | 読み取り専用の解析 | ControllerAnalyzer, ControllerReachability, LayerOwners（レイヤー所有の逆引き）, ParameterUsageFinder, AapWriteScan, BindingOwnership（バインディングごとの「全ステートで書くレイヤー＝所有者」— WD 依存箇所の判定元）, ControllerLocator, StateSearch, ClipDigest（.anim の事実要約 — UI 露出なし、デーモンの exec-method.sh から ClipDigestEntry 経由で叩く） |
