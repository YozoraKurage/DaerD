...
namespace Yozolab.DaerD.Tests
{
    public class RecipeExporterTests
    {
        ...
        [Test]
        public void Export_AsyncSyncLayer_KeepsTheFlags_AndSaysNothingOfTheirWatchers()
        {
            WithSavedController(controller =>
            {
                ApplySync(controller, r =>
                {
                    r.ready = true;
                    r.stale = true;
                    var group = new GraphFrameData.AsyncSyncConfig.SyncGroup { name = "Set" };
                    group.members.AddRange(new[] { "A", "T" });
                    r.groups.Add(group);
                });

                string code = RecipeExporter.Export(controller, null, "SyncRecipe", null).code;
                StringAssert.Contains(".Ready()", code);
                StringAssert.Contains(".Stale()", code);
                StringAssert.Contains(".Group(\"Set\", \"A\", \"T\")", code);

                // The watchers are the same call's output: named as layers that were there,
                // and then left to the call rather than written out as states of their own.
                StringAssert.Contains("// ---- Layer: Zip Ready ", code);
                StringAssert.Contains("// ---- Layer: Zip Stale ", code);
                StringAssert.Contains("// ---- Layer: Zip Set ", code);
                StringAssert.Contains("(regenerated by the async sync layer above)", code);
                StringAssert.DoesNotContain("c.Layer(\"Zip Ready\")", code);
                StringAssert.DoesNotContain("c.Layer(\"Zip Stale\")", code);
                StringAssert.DoesNotContain("\"Zip/Ready\"", code);
                StringAssert.DoesNotContain("\"Zip/Seen/A\"", code);
                StringAssert.DoesNotContain("\"Zip/Fresh/A\"", code);
                StringAssert.DoesNotContain("\"Zip/Hold/A\"", code);
            });
        }
