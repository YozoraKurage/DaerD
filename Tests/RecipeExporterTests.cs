...
namespace Yozolab.DaerD.Tests
{
    public class RecipeExporterTests
    {
        ...
        [Test]
        public void Export_AsyncSyncLayer_KeepsReady_AndSaysNothingOfItsWatcher()
        {
            WithSavedController(controller =>
            {
                ApplySync(controller, r => r.ready = true);

                string code = RecipeExporter.Export(controller, null, "SyncRecipe", null).code;
                StringAssert.Contains(".Ready()", code);

                // The watcher is the same call's output: named as a layer that was there, and
                // then left to the call rather than written out as states of its own.
