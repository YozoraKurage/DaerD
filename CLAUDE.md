...
メインセッション（Fable）は仕様・統括・レビュー担当。実装の割り振りは
（2026-08-07 決定、2026-08-18 改訂）:

- **Fable が直接**: 設計裁定済みで 3 ファイル程度までの修正。委任の固定費
  （仕様・地図・対象読込 + デーモン往復 ≈ 数万トークン）はタスク規模に
  比例しないため、小タスクは委任だと割高。
- **Sonnet**（`model: "sonnet"`）: 機械的で答えが決まっている作業 —
  翻訳抜けの補充、UI 文言、局所削除、テスト追記など。
- **Opus**（`model: "opus"`）: 設計が絡む実装（Engine/Persist/Authoring の意味論、
  直列化データに触るもの）。大きい作業は**クリーンな新規 Opus** に投げ、続けて出る
  軽い追加指示は SendMessage で**同じエージェントへ追送**する。

スペックは読む範囲を閉じる（対象ファイル・シグネチャ・行まで貼る）。小タスクは
2〜3 個束ねて 1 ディスパッチに。コミットゲートのテストはスペックで名指しして絞る。
エージェントには最初に `ARCHITECTURE.md` を読ませ、探索 grep が要ったら地図の
不備として直す。
...
- ユーザー提供の実ファイルは `temp~/` に置く（.gitignore 済み）。末尾のチルダは
  Unity にインポートさせないための慣習で、これが無いと生成 cs がテストプロジェクトの
  コンパイル対象に入ってしまう。原則 temp~/ 以外にコピーしないが、**解析のために
  Unity に読ませる必要があるときだけ**、テストプロジェクト（名前付きボリューム、
  どの git にも属さない）の `Assets/ZZ_UserAssets/` へ複製してよい
  （2026-08-19 ユーザー決定）。解析が済んだら消す。
...
## .anim は YAML を読まず digest で把握する（リバースエンジニアリング補助）

ユーザーのクリップ / コントローラーを解析するとき、Unity YAML を直接読まない。
デーモン起動中なら常駐 Unity に依頼して数秒で事実要約（clip digest）が取れる:

```
echo "Assets/ZZ_UserAssets/Foo.controller" > /tmp/claude-*/…/args.txt   # 1 行 1 パス
.devcontainer/unity/exec-method.sh 'Yozolab.DaerD.Analyze.ClipDigestEntry.Run' <args.txt>
```

- `.anim` は単体の digest、`.controller` は参照される全クリップ + 使用箇所つき。
- 実体は `Editor/Model/Analyze/ClipDigest.cs`（UI 露出なし・public にしない）。
  exec-method.sh は任意の `static string Method(string)` を叩ける汎用チャネル。
- digest は事実の圧縮のみ（値・レンジ・キー数・型分類）。カーブ形状・タンジェント・
  キー時刻は出さないと明言する — 形が要る場合だけ .anim を開く。
